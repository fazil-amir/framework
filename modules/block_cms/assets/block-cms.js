...
var BlockCMS = (function () {
	...
	this.initCollapse = function() {
		$('body').on('click', '.collapse-handle', function () {
			$(this).parents('.block-container').toggleClass('collapse-content');
		})
	}
	...
	this.addTextBlock = function (size, data = '') {
		...
		var html = `
			<div class="block-container text-block" data-block-type="TEXT" >
				${this.getComponentHeader()}
				<div class="row item-wrapper-main">${cols}</div>
			</div>
		`;
	...
	// Error set unset
	this.setErrorMsg = function ($this, value) {
		that.process = false;
		var msg = '';
		switch (value.error) {
