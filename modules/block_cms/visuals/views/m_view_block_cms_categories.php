							...
							<tbody>
								...
								<?php $i = 0; foreach( $data['categories'] as $key => $category ) { ?>						
									...
									<tr id="<?php echo 'parent-' .  $category['cat_id']; ?>" 
										...
										<td style="min-width: 180px;" class="search-here"><?php echo $category['cat_id']; ?></td>							
										<td style="min-width: 200px;" class="search-here editable validate" name="cat_name" minlength="3" maxlength="100" required="required" ><?php echo $category['cat_name']; ?></td>
										<td style="min-width: 250px;" class="search-here editable validate" name="headline" minlength="3" maxlength="100" required="required"><?php echo $category['headline']; ?></td>							
										<td style="min-width: 180px;" class="search-here" style="padding: 0; min-width: 50px;">
										...
										<td class="search-here" style="min-width: 150px;"><?php echo $category['added_by']; ?></td>
										<td style="min-width: 50px;"><?php echo $category['child_count']; ?></td>
										
										<td align="center" style="min-width: 250px;">
											<a class="cms-edit-trigger" href="#" data-form="<?php echo '#parent-' .  $category['cat_id']; ?>" >EDIT</a>
											<a class="cms-more-trigger" href="<?php echo baseURL('panel/block-cms/category/add/' . $category['cat_id']); ?>" >FULL PAGE</a>
											<a class="cms-delete-trigger" href="#" data-count-error-message="Cannot delete this category as posts exist" data-count-child="<?php echo $category['child_count'];?>" id="cms-delete-trigger-<?php echo $category['cat_id']; ?>"  data-form="<?php echo '#parent-' .  $category['cat_id']; ?>">DELETE</a>
										</td>
