...
class ImageSlider extends Widget {
	constructor() {
		super();
		this.slideCounter = 1;
		this.templateLocation = baseURL('modules/widgets/assets/image_slider_tmpl.html');
	...
	renderSlide = (value = null) => {
		this.fetchTemplate().then(tmp => {
			...
			this.handleSlideCollapse(collapseID);
			this.slideCounter++;
		});
	}
	...
	getSliderData = () => {
		...
		for(let i = 0; i < this.slideElements.length; i++){
			...
			data.push({
				...
				target: input[3].checked ? input[3].value : input[4].value,
