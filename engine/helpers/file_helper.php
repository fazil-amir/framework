...
================================================================================================================ */
if (!function_exists('decodeAndCreateFile')) {	
	function decodeAndCreateFile($dir, $baseData){
		$file = base64_decode( preg_replace('#^data:image/\w+;base64,#i', '', $baseData) );
...
if (!function_exists('uploadFile') ) {	
	function uploadFile($attrName, $destination){
		if (isset($_FILES[$attrName]) && $destination) {
			$ext 					= explode('.', $_FILES[$attrName]['name']);
			$filename 		= date('YmdHis') . rand() . '.' . $ext[1];
			$destination 	= $destination . '/' . $filename; //change path of the folder...
			$location 		= $_FILES[$attrName]["tmp_name"];
			$res = move_uploaded_file($location, $destination);
			return [
				'success' => true,
				'message'	=> $filename
			];
		...
		return [
			'success' => false,
			'message'	=> 'No file or destination provided'
		];
	}
}
...
if (!function_exists('copyFile')) {
	function copyFile($src, $dest) {
		...
		$src = str_replace(
			$application['NOW']['appDir'],
			$application['NOW']['rootDir'],
			$src
		);
		rename($src, $dest);
	}
